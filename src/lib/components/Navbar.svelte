...
<!-- Navbar -->
<nav
  class="w-full bg-white/80 backdrop-blur-lg py-3 px-6 md:px-12 flex items-center justify-between font-montserrat sticky top-0 z-50 border-b border-gray-100"
>
  <!-- Logo -->
  <a href="/" class="flex items-center space-x-3 group">
    <div
      class="w-10 h-10 md:w-11 md:h-11 rounded-xl flex items-center justify-center overflow-hidden"
    >
      <img
        src="/favicon.svg"
        alt="Logo"
    ...
    <div class="flex flex-col">
      <span
        class="text-sm md:text-base font-bold tracking-tight text-gray-900 leading-tight"
      >
        Bdn. Sri Debi Utari
      </span>
  ...
  <button
    ...
    class="relative z-50 md:hidden w-10 h-10 flex items-center justify-center rounded-lg hover:bg-gray-50 transition-colors cursor-pointer"
...
{#if mobileOpen}
  ...
  <div
    class="fixed top-0 right-0 h-full w-72 bg-white shadow-2xl z-50 md:hidden p-6 flex flex-col animate-slide-in"
  >
    ...
    <div class="flex flex-col space-y-1 flex-1">
      {#each navItems as item}
        ...
        <a
          href={item.href}
          onclick={() => (mobileOpen = false)}
          class="px-4 py-3 rounded-xl text-sm font-medium transition-colors {isActive
            ? 'text-pink-600 bg-pink-50'
